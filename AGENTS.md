...
- APIExecuter 内置统一准入门禁：非 `Public` 注册方法在 Execute 前经 `api_auth_session.AuthenticateRequest` 鉴权——入口按 `auth_cfg.auth_type` 分发鉴权类型（缺省 `jwt`=self-issuer 自签自验；`bridge`=验外部 IdP 签发的令牌，见下条；非法值 fail-closed 拒绝全部受保护方法），凭证来源（2026-09-11 用户拍板）：协议 Adapter 把本协议的 HTTP `Authorization: Bearer` 头（gRPC 为 `authorization` metadata）经 `api_auth_session.WithBearerToken` 写入 context；调用方也可显式放 `arguments.jwt_token`（优先于头）。验证后即将 `jwt_token` 从 arguments 移除——业务 Execute 与 Async 落库只见业务参数；身份经 context 下传，业务方法用 `api_auth_session.AuthenticatedUser(ctx)` 读取，不得自行鉴权。门禁可经 `auth_cfg.enabled` 开关（缺省启用，段/键缺席均按启用处理）：显式 `enabled: false` 时门禁放行且不提供身份（`jwt_token` 传入仍被移除），依赖 `AuthenticatedUser` 的方法照常返回权限错误——不造匿名/系统假身份。`jwt_token` 入参 schema 由方法注册表按非 Public 恒定注入为可选项（不随开关变化），业务注册声明即启动 panic；MCP Adapter 用 `InputSchemaWithoutGateToken()` 对外描述工具，凭证只走标准 HTTP 头。`SupportedMethod.Public` 零值为受保护（fail-closed），仅 `test`、验证码、注册、登录、refresh 显式标记 `Public: true`。
- `auth_type: bridge` 是身份桥接形态：令牌由外部 IdP 签发，本服务用 IdP 公钥本地验签（`api_auth_bridge`，ES256 纯标准库实现，PKIX PEM/P-256，算法白名单禁 `none` 与 HS 降级），无 session 库表与 DB 用户行——身份即验签出的 claims，业务方法用 `api_auth_session.AuthenticatedBridgePrincipal(ctx)` 读取（与 jwt 形态的 `AuthenticatedUser` 是两种身份形状：令牌 claims vs DB 用户行）。启动期经 `InitializeBridge()` 预加载公钥 fail-fast（`StartAPIServices` 已挂）。**本地令牌吊销是 bridge 的固有职责**：IdP 的 logout 只吊销它那侧的 refresh token，access token 在本地验签下仍有效到自然过期，这一段只有本服务能拦——`auth_cfg.bridge.revocation_enabled`（缺省开）按用户维度记录登出时刻（令牌 `iat` 早于登出时刻即拒；查询失败 fail-closed）；`RevocationStore` 由实例注入（`SetupRevocation`），模板自带进程内 `MemoryRevocationStore` 供单实例/测试，多实例部署必须换共享存储实现。本形态源自 certa_cg 实例真联调验证后的回灌。
- api 层（协议 Adapter、APIExecuter、准入门禁、`api_auth` 域）定型后不再随业务演进修改；新增功能一律进 `ability` 业务子包。契约明记的既有例外：`SupportedMethod.Async` 受理语义的一次性接入，`ability_user_profile` 的顶层装配（profile 依赖父包 `ability_user` 数据方法，父包带子包装配会成 import 环），以及 2026-09-11 用户拍板的凭证入口改造（Adapter 透传 `Authorization` 头 + 监听地址配置化）。
- 监听地址是必填配置：`api_cfg.api_cfg_{jsonRPC,mcp,websocket,grpc}.bind_address` 缺失或已启用端口非法时 `config.ApiConfig.Validate()` 在启动期拒绝，代码里不兜底 `127.0.0.1` / `0.0.0.0`；生成的缺省配置只监听本机，对外部署由运维改成 `0.0.0.0`。
